...
shap_table <- read.csv('sdonly_shap_table.csv')
...
shap_data1 <- read.csv('sdonly_shapdata1.csv')
...
#x_mat1 <- model.matrix(~-1 + ., x)
 ...
 o1_labels <- c( 
   ...
   'Outpx pain dx 12m ',
   '# months w/ polypharmacy ',
   '# outpx alcohol dx visits 4yr',
   'Musculoskeletal disease dx 12m ',
   'Lifetime suicide attempt',
   'ACEs: Felt hated by family',
   'Command: Guard or Reserve',
   'Age left Army',
   'Nicotine dependence dx 4yr ',
   'Honorable discharge',
   'Duty MOS ',
   'Outpx alcohol dx 4yr ',
   'Last AFQT score'
 )
