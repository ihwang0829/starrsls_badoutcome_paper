...
setwd('/home/data/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/Update_202503/Analysis/RESULTS/risk_hlonly/')
...
saveRDS(cv_lasso,'risk_hlonly.rds')
...
write.csv(lasso_coefs,'hlonly_lasso_coefs.csv',row.names=F)
write.csv(shap_data,'hlonly_shapdata.csv',row.names=F)
...
saveRDS(xgbFit,'xgbFit_hlonly.rds')
...
write.csv(xgb_shap,'hlonly_xgb_shap.csv',row.names=F)
write.csv(shap_table,'hlonly_shap_table.csv',row.names=F)