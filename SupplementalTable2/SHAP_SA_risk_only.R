...
#setwd('O:/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/Update_202503/Analysis/RESULTS/risk_saonly/')
...
setwd('/home/data/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/Update_202503/Analysis/RESULTS/risk_saonly/')
indata <- read_sas('/home/data/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/data/allvars_for_shap.sas7bdat') 



sampleweight <- indata$weight_lsw3_or_lsw4_norm1

skip <- c('MASTERID','secu','str','weight_lsw3_or_lsw4_norm1','fold','riskcat','risk_saonly','risk_hlonly','risk_sdonly','risk_2plus')


Y <- indata$risk_saonly 
...
saveRDS(cv_lasso,'risk_saonly.rds')

#cv_lasso <- readRDS('risk_saonly.rds')
...
write.csv(lasso_coefs,'saonly_lasso_coefs.csv',row.names=F)
write.csv(shap_data,'saonly_shapdata.csv',row.names=F)
...
saveRDS(xgbFit,'xgbFit_saonly.rds')

#xgbFit <- readRDS('xgbFit_saonly.rds')
#shap_data<-read.csv('saonly_shapdata.csv')
...
#apply(shap_table, 2, function(x) mean(abs(x)))
 
write.csv(xgb_shap,'saonly_xgb_shap.csv',row.names=F)
write.csv(shap_table,'saonly_shap_table.csv',row.names=F)