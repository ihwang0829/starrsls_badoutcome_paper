...
setwd('/home/data/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/Update_202503/Analysis/RESULTS/SA_outcome/')
...
saveRDS(cv_lasso,'SA_outcome.rds')

#cv_lasso <- readRDS('SA_outcome.rds')
...
write.csv(lasso_coefs,'SA_outcome_lasso_coefs.csv',row.names=F)
write.csv(shap_data,'SA_outcome_shapdata.csv',row.names=F)
...
saveRDS(xgbFit,'xgbFit_SA_outcome.rds')
...
write.csv(xgb_shap,'SA_outcome_xgb_shap.csv',row.names=F)
write.csv(shap_table,'SA_outcome_shap_table.csv',row.names=F)