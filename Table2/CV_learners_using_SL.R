...
source('WeightedBrierScore.R')
source('output_ici.R')
source('adj_pred.R')

setwd('/working/directory/')

cvAUCs <- read_sas('cvAUCs.sas7bdat')
keep_vars <- cvAUCs$var[unique(which(cvAUCs$cvAUC >= 0.51 | (cvAUCs$cvAUC - (1.96 * cvAUCs$cvSE) > 0.5)))]

full_data <- read_sas('full_data.sas7bdat')
...
n_real_Y <- length(unique(full_data$ID[Y == 1,]))  # Person-level count 
...
SL.library <- c(list(c(lrnr.lasso,'screen.corr')),
                grid.glmnet.screen,
...
saveRDS(sl,'cv_learners_using_sl.rds')
...
out_pred <- data.frame(
  full_data[,c('ID','fold','weight','Y')],
  cv_preds
)

write.csv(out_pred,'cv_preds.csv',row.names=F)
write.csv(lrnr_xwalk,'learner_xwalk.csv',row.names=F)
...
# As an example, let's say the best nnet was the 17th learner
cv_pred_nnet <- cv_preds[,which(lrnr_xwalk$var == 'lrnr17')] %>% as.numeric()  
