...
saveRDS(sl,'emn_cv_learners_using_sl.rds')

sl <- readRDS('emn_cv_learners_using_sl.rds')
...
out_pred <- data.frame(
  full_data[,c('masterid','fold','unempoutcome','weight_lsw3_or_lsw4_norm1')],
  cv_preds
)

write.csv(out_pred,'emn_cv_preds.csv',row.names=F)
write.csv(lrnr_xwalk,'emn_learner_xwalk.csv',row.names=F)
...
#   best elastic net  
cv_pred_glmnet <- cv_preds[,which(lrnr_xwalk$var == 'lrnr7')] %>% as.numeric()  

#   best nnet  
cv_pred_nnet <- cv_preds[,which(lrnr_xwalk$var == 'lrnr40')] %>% as.numeric()  

#   best xgboost  
cv_pred_xgboost <- cv_preds[,which(lrnr_xwalk$var == 'lrnr64')] %>% as.numeric()  

#   best random forest  
cv_pred_ranger <- cv_preds[,which(lrnr_xwalk$var == 'lrnr55')] %>% as.numeric()  

#   best wsvm  
cv_pred_wsvm <- cv_preds[,which(lrnr_xwalk$var == 'lrnr37')] %>% as.numeric()  

#   best polymar  
cv_pred_poly <- cv_preds[,which(lrnr_xwalk$var == 'lrnr18')] %>% as.numeric()  
