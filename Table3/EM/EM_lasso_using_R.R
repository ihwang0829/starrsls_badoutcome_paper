...
saveRDS(sl,'emall_lasso_sl.rds')


###############################
# Export pred probs for CV-AUC
###############################
emcv_pred_lasso <- as.numeric(sl$Z[,1])

out_pred <- data.frame(
  full_data[,c('masterid','stack','fold','weight_lsw3_or_lsw4_norm1','unempoutcome')],
  emcv_pred_lasso = emcv_pred_lasso
)

write.csv(out_pred,'emcv_pred_lasso.csv',row.names=F)
...
Brier_lasso <- lapply(
  validRows,
  function(i)
    WeightedBrierScore(x = Y[i], pred = emcv_pred_lasso[i], weight = weight[i])
) %>% unlist()
...
ici_lasso <- lapply(
  validRows,
  function(i)
    output_ici(Y=Y[i], pred=emcv_pred_lasso[i], pop_weight=weight[i])
)
...
emcv_pred_lasso_pq <- adj_pred(emcv_pred_lasso, p_pop = prev, w = weight)

Brier_lasso_pq <- lapply(
  validRows,
  function(i)
    WeightedBrierScore(x = Y[i], pred = emcv_pred_lasso_pq[i], weight = weight[i])
) %>% unlist()
...
ici_lasso_pq <- lapply(
  validRows,
  function(i)
    output_ici(Y=Y[i], pred=emcv_pred_lasso_pq[i], pop_weight=weight[i])
)
