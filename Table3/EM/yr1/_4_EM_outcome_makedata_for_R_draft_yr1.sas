...
libname out "&pathO./Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/data/cvtemps/EM/yr1/";
...
%LET output2=&pathO./Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/data/cvtemps/EM/yr1/; 
...
data  full_data_with_fold ;
 set sepfile.full_data_with_fold_EM;
 if stack=1;
run;
...
combine 
dischargetype_cat3 and dischargetype_cat4 
*/
...
if dischargetype_cat3=1 or dischargetype_cat4=1 then dischargetype_cat3=1; else dischargetype_cat3=0;
run;
...
data numouts;
 set fullfile1;
 if unempoutcome=1 and &varname >=1;
run; 
proc contents;
...
data sepfile._em_varlist_forcvauc;
 set __varnamelist;
run;
...
proc sql;
 ...
 into: emauc_varlist
 separated by ' '
 from sepfile._em_varlist_forcvauc; 
quit; 
%put &emauc_varlist;

data out._em_forcvauc;
 set fullfile1 (keep= masterid  stack  unempoutcome  weight_lsw3_or_lsw4_norm1  str  secu  nowgt  fold 
                             &emauc_varlist);
run;
...
%let DSET_MODEL= out._em_forcvauc; **<-- USER MUST EDIT; 
...
%let DEPVAR = unempoutcome ; **<-- USER MUST EDIT; *dichotomous outcome variable ; 
...
** OUTPUT DATA SETS: ;
%let OUT_Converge_fitted_ALL = OUT.em_lst_converge_st_fit_all ; **<- convergence status of all fitted models ; 

%let OUT_UNIV_FITTED = OUT.em_univ_model_fitted ; **<- output data set for all logistic model parameters;
												**optional data set for troubleshooting ;

%let OUT_UNIV_SCORED =  OUT.em_univ_model_scored ; **<- output data set for all predicted probabilities;
												** optional data set for troubleshooting;

%let OUT_AUC_VFOLDS_ALL = out.em_univ_vfold_auc_all ; **<-- output data set for validation fold AUCs for univariate model ; 
%let OUT_AUC_STATS =  OUT.em_univ_model_auc_stats ;  **<- output data set for auc contrast pvalues ;
...
data out.em_auc_basemodel_fullsample ; length ROCModel $12 controls $60 N Sum_Weights 8 ;
merge 
...
proc sort data=out.em_univ_model_auc_stats  out=aucstats; BY DESCENDING cv_AUC;
...
proc contents data=out._em_forcvauc;
ods output variables=_cont;
proc sort ; by variable;
run;
data aucstats1;
 retain outcome	Predictor	Label	cv_AUC	SE_cv_AUC	lb	keep;
 merge aucstats1 (in=a) _cont (keep=variable label rename=(variable=predictor));
 by predictor;
if a;
...
ODS HTML STYLE=sasweb FILE="&output2._EM_totalsamp_cv_AUC.html";
proc print data=aucstats1 ;
run; 
ODS HTML CLOSE;

data out.em_univ_model_auc_stats_f;
 set aucstats1;
run; 

data out.em_cvaucs (keep=predictor cv_auc keep);
set out.em_univ_model_auc_stats_f;
if keep=1;
run;

proc print data=out.em_cvaucs ;
run;
