...
cvsl <- readRDS('cvsl.rds')
cvslcoef <- coef(cvsl)

write.csv(cvslcoef, file =  "SL-coef_slcvS.csv" )
...
out_predcvsl <- data.frame(
  train_data[,c('masterid','stack','fold','weight_lsw3_or_lsw4_norm1','hl_final')],
  cv_pred = cv_pred
)

write.csv(out_predcvsl,'cv_pred_cvsl.csv',row.names=F)
