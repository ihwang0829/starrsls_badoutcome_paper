...
#setwd('O:/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/data/cvtemps/HL/')
...
setwd('/home/data/Harvard/Hwang/STARRSLS/AnalyticFiles_NEW/6_LSW4_variables/Separation_analysis_makedata/data/cvtemps/HL/') 
...
saveRDS(sl,'hln_cv_learners_using_sl.rds')
#sl <- readRDS('hln_cv_learners_using_sl.rds')
...
out_pred <- data.frame(
  full_data[,c('masterid','fold','hl_final','weight_lsw3_or_lsw4_norm1')],
  cv_preds
)

write.csv(out_pred,'hln_cv_preds.csv',row.names=F)
write.csv(lrnr_xwalk,'hln_learner_xwalk.csv',row.names=F)
...
#   best elastic net  
cv_pred_glmnet <- cv_preds[,which(lrnr_xwalk$var == 'lrnr6')] %>% as.numeric()  

#   best nnet  
cv_pred_nnet <- cv_preds[,which(lrnr_xwalk$var == 'lrnr43')] %>% as.numeric()  

#   best xgboost  
cv_pred_xgboost <- cv_preds[,which(lrnr_xwalk$var == 'lrnr87')] %>% as.numeric()  

#   best random forest  
cv_pred_ranger <- cv_preds[,which(lrnr_xwalk$var == 'lrnr55')] %>% as.numeric()  

#   best wsvm  
cv_pred_wsvm <- cv_preds[,which(lrnr_xwalk$var == 'lrnr35')] %>% as.numeric()  
