...
outcome file
        1    masterid                     Char     20    $20.      $20.        MASTERID
        2    stack                        Num       8
        3    hl_final                     Num       8
        4    weight_lsw3_or_lsw4_norm1    Num       8
        5    str                          Num       8
        6    secu                         Num       8
        7    nowgt                        Num       8
...
/*
%macro bystack(stack=,);
proc sort data=sepfile._outcomesf_HL (where=(stack=&stack)) out=hloutcome;
 by masterid ;
run;
...
PROC SQL NOPRINT;
	CREATE TABLE _person_level AS
		SELECT masterid
			,MAX(hl_final) AS outcome
			,SUM(weight_lsw3_or_lsw4_norm1) AS n
			,RANUNI(250220) AS _rand
		FROM hloutcome
		GROUP BY masterid
		ORDER BY outcome, n DESC, _rand;
QUIT;
...
DATA _person_level2;
	...
	_n + 1;
	IF FIRST.outcome THEN _n = 1;
	
	fold = MOD(_n,20);
	IF fold > 10 THEN fold = 21 - fold;
	ELSE IF fold = 0 THEN fold = 1;
RUN;

PROC SQL NOPRINT;
	CREATE TABLE full_data_with_fold&stack AS
		SELECT a.*
			,b.fold
		FROM hloutcome AS a
		LEFT JOIN _person_level2 AS b
		ON a.masterid = b.masterid;
QUIT;


proc freq data=full_data_with_fold&stack;
tables fold;
run;
proc sort data=full_data_with_fold&stack; by fold;
proc freq data=full_data_with_fold&stack;
tables hl_final;
...
%bystack(stack=2)
%bystack(stack=3)
data full_data_with_fold; set full_data_with_fold1 full_data_with_fold2 full_data_with_fold3;
run; */

 

proc sort data=sepfile._outcomesf_HL out=hloutcome;
 by masterid ;
run;
...
PROC SQL NOPRINT;
	CREATE TABLE full_data_with_fold AS
		SELECT a.*
			,b.fold
		FROM hloutcome AS a
		LEFT JOIN _person_level2 AS b
...
proc freq data=full_data_with_fold;
tables stack*fold*hl_final
/list missing; 
run; 
...
/*
data testing2;
 set full_data_with_fold;
if stack=3 and fold=6 and hl_final=1;
run;
...
run;
proc freq;
tables masterid*stack*fold*hl_final
/list missing;
run;

M210237                     2       6           0           1        8.33             1         8.33
M210237                     3       6           1           2       16.67             3        25.00
M233115                     2       6           0           1        8.33             4        33.33
M233115                     3       6           1           2       16.67             6        50.00
M246600                     2       6           0           1        8.33             7        58.33
M246600                     3       6           1           2       16.67             9        75.00
M269368                     2       6           0           1        8.33            10        83.33
M269368

 manually coding 2 of these to fold 4 and fold 7
*/
data full_data_with_fold;
 set full_data_with_fold;
 if masterid="M210237" then fold=4;
 if masterid="M269368" then fold=7;
...
data sepfile.full_data_with_fold_HL;
 set full_data_with_fold;
run;
 /*
%macro testdups(fold);
data t&fold; set full_data_with_fold ; if fold=&fold; proc sort; by masterid;run;
%mend;
%testdups(1)
%testdups(2)
%testdups(3)
...
if a10 then f10=1;
proc freq;
tables f1*f2*f3*f4*f5*f6*f7*f8*f9*f10
/list missing;
run;
*/
/*
check that same masterids in the same fold;

data testwithdupl;
 set full_data_with_fold;
if masterid="M100237" and stack=2 then fold=7;
run;
%macro splitfold(fold);

data f&fold;
 set full_data_with_fold (where=(fold=&fold));
run;
...
data ffolds;
 merge f1 (in=a1 keep=masterid fold)
       f2 (in=a2 keep=masterid fold)
       f3 (in=a3 keep=masterid fold)
       f4 (in=a4 keep=masterid fold)
       f5 (in=a5 keep=masterid fold)
       f6 (in=a6 keep=masterid fold)
       f7 (in=a7 keep=masterid fold)
       f8 (in=a8 keep=masterid fold)
       f9 (in=a9 keep=masterid fold)
       f10 (in=a10 keep=masterid fold);
by masterid;
if a1 then ia1=1;
if a2 then ia2=1;
if a3 then ia3=1;
...
if a10 then ia10=1; 
proc freq;
tables fold*ia1*ia2*ia3*ia4*ia5*ia6*ia7*ia8*ia9*ia10
/list missing;
run;
...
data f&fold;
 set testwithdupl (where=(fold=&fold));
run;
proc sort; by masterid;
run;
%mend;
...
data  full_data_with_fold ;
 set sepfile.full_data_with_fold_HL;
run;
...
/*
 <5 warrant officer with outcome, combine commission and warrant officer
*/

data fullfile1;
 set fullfile;
if rank_admin in (3,4) then rank_admin=3;
...
data sepfile._hl_varlist_forcvauc;
 set __varnamelist;
run;
...
proc sql;
 ...
 into: hlauc_varlist
 separated by ' '
 from sepfile._hl_varlist_forcvauc; 
quit; 
%put &hlauc_varlist;

data out._hl_forcvauc;
 set fullfile1 (keep= masterid  stack  hl_final  weight_lsw3_or_lsw4_norm1  str  secu  nowgt  fold 
                             &hlauc_varlist);
run;
...
%let DSET_MODEL= out._hl_forcvauc; **<-- USER MUST EDIT; 
...
%let DEPVAR = hl_final ; **<-- USER MUST EDIT; *dichotomous outcome variable ; 
...
** OUTPUT DATA SETS: ;
%let OUT_Converge_fitted_ALL = OUT.hl_lst_converge_st_fit_all ; **<- convergence status of all fitted models ; 

%let OUT_UNIV_FITTED = OUT.hl_univ_model_fitted ; **<- output data set for all logistic model parameters;
												**optional data set for troubleshooting ;

%let OUT_UNIV_SCORED =  OUT.hl_univ_model_scored ; **<- output data set for all predicted probabilities;
												** optional data set for troubleshooting;

%let OUT_AUC_VFOLDS_ALL = out.hl_univ_vfold_auc_all ; **<-- output data set for validation fold AUCs for univariate model ; 
%let OUT_AUC_STATS =  OUT.hl_univ_model_auc_stats ;  **<- output data set for auc contrast pvalues ;
...
data out.hl_auc_basemodel_fullsample ; length ROCModel $12 controls $60 N Sum_Weights 8 ;
merge 
...
proc sort data=out.hl_univ_model_auc_stats  out=aucstats; BY DESCENDING cv_AUC;
...
proc contents data=out._hl_forcvauc;
...
ODS HTML STYLE=sasweb FILE="&output2._HL_totalsamp_cv_AUC.html";
proc print data=aucstats1 ;
run; 
ODS HTML CLOSE;

data out.hl_univ_model_auc_stats_f;
 set aucstats1;
run; 

data out.hl_cvaucs (keep=predictor cv_auc keep);
set out.hl_univ_model_auc_stats_f;
if keep=1;
run;

proc print data=out.hl_cvaucs ;
run;
