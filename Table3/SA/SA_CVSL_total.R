...
saveRDS(cvsl,'sacvsl.rds')
  
cvsl<-readRDS('sacvsl.rds')

screen_nvars <- rowSums(cvsl$AllSL[[1]]$whichScreen)

#> screen_nvars
#All  screen.corr.lasso.1  screen.corr.lasso.2  screen.corr.lasso.3 screen.corr.ranger.1 screen.corr.ranger.2 
#84                   32                   21                   21                   25                   50 
#screen.corr.ranger.3 
#78 

for(i in 2:length(cvsl$AllSL))
  screen_nvars <- rbind(screen_nvars, rowSums(cvsl$AllSL[[i]]$whichScreen))

apply(screen_nvars, 2, min)
apply(screen_nvars, 2, max)

#For the hyperparameters, you can add this to the end of the CVSL R code:
  
  cvsl_meta_wts <- t(coef(cvsl))
cvsl_meta_wts <- cvsl_meta_wts[which(rowSums(cvsl_meta_wts) > 0),]

selected_glmnets <- grep('SL.glmnet',row.names(cvsl_meta_wts),value=T)

glmnet_names <- stringr::str_split(selected_glmnets[1],'_screen')[[1]][1]
for(i in 2:length(selected_glmnets))
  glmnet_names <- c(glmnet_names,stringr::str_split(selected_glmnets[i],'_screen')[[1]][1])
glmnet_names <- unique(glmnet_names)

glmnet_hyp <- grid.glmnet$grid[grid.glmnet$names %in% glmnet_names,,FALSE]
row.names(glmnet_hyp) <- grid.glmnet$names[grid.glmnet$names %in% glmnet_names]
glmnet_hyp
...
selected_ranger <- grep('SL.ranger',row.names(cvsl_meta_wts),value=T)

ranger_names <- stringr::str_split(selected_ranger[1],'_screen')[[1]][1]
for(i in 2:length(selected_ranger))
  ranger_names <- c(ranger_names,stringr::str_split(selected_ranger[i],'_screen')[[1]][1])
ranger_names <- unique(ranger_names)


ranger_hyp <- grid.ranger$grid[grid.ranger$names %in% ranger_names,FALSE]


row.names(ranger_hyp) <- grid.ranger$names[grid.ranger$names %in% ranger_names]
ranger_hyp
...
screen.corr.lasso.2
screen.corr.lasso.3
screen.corr.ranger.1
screen.corr.ranger.2
screen.corr.ranger.3

grid.xgb$grid$ntrees
grid.xgb$grid$max_depth
grid.xgb$grid$shrinkage
grid.xgb$grid$gamma
grid.xgb$grid$minobspernode
grid.xgb$grid$colsample_bytree
