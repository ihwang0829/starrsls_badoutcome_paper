...
write.csv(cvslcoef, file =  "saSL-coef_slcvS.csv" )
...
out_predcvsl <- data.frame(
  train_data[,c('masterid','stack','fold','weight_lsw3_or_lsw4_norm1','SA_outcomef')],
  cv_pred = cv_pred
)

write.csv(out_predcvsl,'sacv_pred_cvsl.csv',row.names=F)
