...
%MACRO calc_lasso(type=,dir=,pre=,outcome=, pp=, csv=, outfile1=, outfile2=, stack=, );
...
PROC IMPORT OUT= WORK.ppfile 
            DATAFILE= "&output2.&csv..csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
proc sort data=out._&pre._forcvauc out=&pre.dat;
 by masterid stack;

data &pre.datnew (keep= masterid stack str  secu  nowgt &outcome weight_lsw3_or_lsw4_norm1  fold   masterid stack str  secu  nowgt &pp  );
 merge &pre.dat 
       ppfile (keep=masterid stack &pp)    ;
 by masterid stack;
...
data &pre.datnew (keep= masterid stack str  secu  nowgt &outcome weight_lsw3_or_lsw4_norm1  fold   masterid stack str  secu  nowgt &pp  );
 merge &pre.dat _&pre._lasso_pp(keep=masterid stack &pp)   ;
 by masterid stack;
run;

*/
%runcvauc_smacro(indat=&pre.datnew, depvar=&outcome, weight=weight_lsw3_or_lsw4_norm1, fold=fold, dropvars=masterid stack str  secu  nowgt, pre=&pre );

data &pre._univ_model_auc_stats1;
...
ODS HTML STYLE=sasweb FILE="&output._&outfile1..html";
proc print data=&pre._univ_model_auc_stats1;
 var cv_AUC SE_cv_AUC auc_lb auc_ub;
run;
proc print data=&pre._univ_model_auc_stats; 
 run;
ODS HTML CLOSE;
/*
%MACRO foldAUCs;
%DO i = 1 %TO 10;
    PROC LOGISTIC DATA=&pre.datnew
        ROCOPTIONS(WEIGHTED);
...
proc univariate data = &pre.datnew_f&fold;
weight  weight_lsw3_or_lsw4_norm1;
var &pp;
...
%mend;
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=1);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=2);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=3);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=4);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=5);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=6);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=7);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=8);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=9);
%cvrisks(indata=&pre.datnew, ppvar=&pp, fold=10);
...
tier+1;
 run;
ODS HTML STYLE=sasweb FILE="&output._&outfile2..html";
...
%calc_lasso(type=lasso,dir=HL/,    pre=hl,outcome=hl_final,         pp=cv_pred_lasso,   csv=cv_pred_lasso,   outfile1=_s1_hl_total_ppauc, outfile2=_s1_hl_total_CRtables, stack=1)
%calc_lasso(type=lasso,dir=HL/,    pre=hl,outcome=hl_final,         pp=cv_pred_lasso,   csv=cv_pred_lasso,   outfile1=_s2_hl_total_ppauc, outfile2=_s2_hl_total_CRtables, stack=2)
%calc_lasso(type=lasso,dir=HL/,    pre=hl,outcome=hl_final,         pp=cv_pred_lasso,   csv=cv_pred_lasso,   outfile1=_s3_hl_total_ppauc, outfile2=_s3_hl_total_CRtables, stack=3) 

%calc_lasso(type=lasso,dir=SA/,    pre=sa,outcome=SA_outcomef,      pp=sacv_pred_lasso, csv=sacv_pred_lasso, outfile1=_s1_sa_total_ppauc, outfile2=_s1_sa_total_CRtables, stack=1) 
%calc_lasso(type=lasso,dir=SA/,    pre=sa,outcome=SA_outcomef,      pp=sacv_pred_lasso, csv=sacv_pred_lasso, outfile1=_s2_sa_total_ppauc, outfile2=_s2_sa_total_CRtables, stack=2) 
%calc_lasso(type=lasso,dir=SA/,    pre=sa,outcome=SA_outcomef,      pp=sacv_pred_lasso, csv=sacv_pred_lasso, outfile1=_s3_sa_total_ppauc, outfile2=_s3_sa_total_CRtables, stack=3) 
 
%calc_lasso(type=lasso,dir=EM/,    pre=em,outcome=unempoutcome,     pp=emcv_pred_lasso, csv=emcv_pred_lasso, outfile1=_s1_em_total_ppauc, outfile2=_s1_em_total_CRtables, stack=1) 
%calc_lasso(type=lasso,dir=EM/,    pre=em,outcome=unempoutcome,     pp=emcv_pred_lasso, csv=emcv_pred_lasso, outfile1=_s2_em_total_ppauc, outfile2=_s2_em_total_CRtables, stack=2) 
%calc_lasso(type=lasso,dir=EM/,    pre=em,outcome=unempoutcome,     pp=emcv_pred_lasso, csv=emcv_pred_lasso, outfile1=_s3_em_total_ppauc, outfile2=_s3_em_total_CRtables, stack=3) 
